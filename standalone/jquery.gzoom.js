...
jQuery.fn.gzoom = function(settings) {
    ...
    windowH:10, // window height
    windowW:10, // window width
	...
	return this.each(function(){
	  ...
	  var swapped = false;
		...
		var $div = jQuery(this).css({width: settings.windowW, height: settings.windowH, border: settings.frameWidth+"px solid "+settings.frameColor, overflow:'hidden'}).addClass("minizoompan");
		$div.wrap('<div class="gzoomwrap"></div>').css({ width:settings.windowW });
        ...
        $('.preview').on('dragstart', function(event) { event.preventDefault(); });
		...
		$div.mousemove(function(e){
          if(!$('#pixastic_crop_ok').is(":visible") && clicking) {
			var divWidth = $div.width();
			var divHeight = $div.height();
			var igW = ig.width();
			var igH = ig.height();
			var dOs = $div.offset();
//			var leftPan = (-(igW/-2)-(e.pageX - dOs.left) * (divWidth - igW)) / (divWidth+settings.frameWidth*2);
//			var leftPan = (e.pageX - dOs.left) * (divWidth - igW) / (divWidth+settings.frameWidth*2);
			var leftPan = (igW/2-(e.pageX - dOs.left)) * (divWidth - igW) / (divWidth+settings.frameWidth*2);
			var topPan = (igH/2-(e.pageY - dOs.top)) * (divHeight - igH) / (divHeight+settings.frameWidth*2);
